...
numeric_columns <- MISSING_DATA[, c("GDP", "PovertyHeadCount", "EducationEnrolment", "UnemploymentRate", "PopulationGrowthRate", "LifeExpectancy")]
...
boxplot(numeric_columns, col = ifelse(outliers, "red", "blue"), 
        ...
        names = c("GDP", "PovertyHeadCount", "EducationEnrolment", "UnemploymentRate", "PopulationGrowthRate", "LifeExpectancy"))
...
for (col_name in numeric_columns) {
  col_values <- MISSING_DATA[[col_name]]
  ...
  if (any(!is.na(outliers) & outliers)) {
    ...
    MISSING_DATA[outliers, col_name] <- median_val
    
    # Print new median
    print(paste("New median:", median(MISSING_DATA[[col_name]], na.rm = TRUE)))
  }
}
...
# Numeric columns
numeric_columns <- c("GDP", "EducationEnrolment", "PovertyHeadCount", "UnemploymentRate", "PopulationGrowthRate", "LifeExpectancy")
...
for (col_name in numeric_columns) {
  ...
  if (any(!is.na(outliers) & outliers)) {
    ...
    # Replace outliers with median
    MISSING_DATA[outliers, col_name] <- as.numeric(median_val)
...
means <- colMeans(MISSING_DATA[, 3:8], na.rm = TRUE)
MISSING_DATA = as.data.frame(lapply(MISSING_DATA, function(x) ifelse(is.na(x), means, x)))

missing_counts <- colSums(is.na(MISSING_DATA))
print(missing_counts)

# Line plot for GDP over time
ggplot(MISSING_DATA, aes(x = Year, y = GDP, group = Country, color = Country)) +
  geom_line() +
  theme(plot.title = element_text(hjust = 0.5))+
...
ggplot(MISSING_DATA, aes(x = Year, y = LifeExpectancy,group = Country, color = Country)) +
  geom_line() +
  theme(plot.title = element_text(hjust = 0.5))+
...
attach(data)
ggplot(MISSING_DATA, aes(x = PovertyHeadCount, y = GDP))+
  geom_point() +
...
ggplot(MISSING_DATA, aes(x = Country, y = PopulationGrowthRate, color = Country)) +
...
MISSING_DATA$Year <- as.factor(MISSING_DATA$Year)
ggplot(MISSING_DATA, aes(x = Year, y = EducationEnrolment,group = Country, color = Country)) +
  geom_line() +
  theme(plot.title = element_text(hjust = 0.5))+
...
correlation_matrix <- cor(MISSING_DATA[,c("GDP", "PovertyHeadCount", "EducationEnrolment", "UnemploymentRate", "PopulationGrowthRate", "LifeExpectancy")])
correlation_matrix

# Hypothesis 1
# Create two vectors for life expectancy (LE) based on PH groups
le_below_average <- MISSING_DATA$LifeExpectancy[MISSING_DATA$PovertyHeadCount < mean(MISSING_DATA$PovertyHeadCount)]
le_above_average <- MISSING_DATA$LifeExpectancy[MISSING_DATA$PovertyHeadCount >= mean(MISSING_DATA$PovertyHeadCount)]
...
mean_gdp <- mean(MISSING_DATA$GDP)
...
high_gdp_countries <- MISSING_DATA[MISSING_DATA$GDP >= threshold, ]
low_gdp_countries <- MISSING_DATA[MISSING_DATA$GDP < threshold, ]

# Perform a two-sample t-test to compare EEL between the two groups
t_test_result <- t.test(high_gdp_countries$EducationEnrolment, low_gdp_countries$EducationEnrolment)
...
data = MISSING_DATA
X <- as.matrix(data[, c("EducationEnrolment", "PovertyHeadCount", "UnemploymentRate", "PopulationGrowthRate", "LifeExpectancy")])
...
ts_data <- ts(data[, c("GDP", "PovertyHeadCount", "UnemploymentRate")], start = min(data$Year), frequency = 1)  # Assuming yearly data
...
for (col_name in numeric_columns) {
  if (col_name %in% colnames(MISSING_DATA)) {
    col_values <- MISSING_DATA[, col_name]
    ...
    if (any(outliers)) {
      ...
      MISSING_DATA[, col_name] <- col_values
